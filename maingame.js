...
function ensurePlayerSetup() {
  ...
  const playerNodeMissing = isParticipant && !gameState[playerName];
  ...
  } else if (gameState.status === "play" && playerNodeMissing) {
...
function openSelectionModal(type, titleOverride) {
  ...
  if (playerData?.playing11?.players) {
    renderSelectionTable(playerData.playing11.players, isBattingSelection);
    elements.selectionModal.classList.remove("hidden");
    elements.selectionModal.classList.add("active");
    return;
  }

  gameRef.child(`${playerName}/playing11`).once("value").then(snapshot => {
    if (!snapshot.exists() || !snapshot.val().players) {
      showToast("Playing 11 not ready.");
      return;
    }
    renderSelectionTable(snapshot.val().players, isBattingSelection);
    elements.selectionModal.classList.remove("hidden");
    elements.selectionModal.classList.add("active");
  });
}

function renderSelectionTable(players, isBattingSelection) {
  ...
  Object.entries(players).forEach(([id, player]) => {
    ...
    const strength = Array.isArray(player.strength) ? player.strength.join(", ") : (player.strength || "-");
    const weakness = Array.isArray(player.weakness) ? player.weakness.join(", ") : (player.weakness || "-");
    row.innerHTML = `
